...
navigator.getUserMedia({video: true}, function (stream) {
	...
	videoElement.addEventListener("canplay", function() {
		...
		var currentImageData = greyscaleCtx.createImageData(horizontalResolution, verticalResolution);
		...
		function analyzeCurrentFrame() {
			...
			var currentBrushColor = hsl2rgb(hue, Math.round(data[0] / 2), Math.round(100 - (data[0] * 0.25)));
			...
			greyscaleCtx.drawImage(videoElement, 0, 0, horizontalResolution, verticalResolution, 0, 0, horizontalResolution, verticalResolution);

			// Start the process of desaturating it
			var imageData = greyscaleCtx.getImageData(0, 0, horizontalResolution, verticalResolution);
			var displayImageData = displayCanvasContext.getImageData(0, 0, horizontalResolution, verticalResolution);
			...
			for (var i = 0; i < dataLength; i += 4) {
				...
				if (Math.abs(previousData[i] - newData[i]) > PIXEL_CHANGE_THRESHOLD) {
					...
					displayData[i+1] = displayData[i+1] < 240 ? displayData[i+1] + 15 : 255;
					displayData[i+2] = displayData[i+2] < 240 ? displayData[i+2] + 15 : 255;
				}
			}
